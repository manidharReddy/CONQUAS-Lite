...
@synthesize txtComponentJPTicks;
@synthesize txtComponentJPChecks;
...
@synthesize txtDoorJPTicks;
@synthesize txtDoorJPChecks;
...
@synthesize txtMEJPTicks;
@synthesize txtMEJPChecks;
...
- (void)viewDidLoad
{
    ...
    if ([filemgr fileExistsAtPath:databasePath] == NO)
    {
        ...
        {
            char *errMsg;
            const char *strSQL = "CREATE TABLE IF NOT EXISTS InternalFinishesDetails(ID INTEGER PRIMARY KEY AUTOINCREMENT, blkNumber TEXT, storeyNumber TEXT, unitNumber TEXT, editionNumber TEXT, locationName TEXT, elementType TEXT, defectType TEXT, numTicks INTEGER, numChecks INTEGER, projectID TEXT, projectName TEXT, creationDate DATETIME default current_timestamp)";
            ...
            strSQL = "CREATE TABLE IF NOT EXISTS InternalFinishesSummaryDetails(ID INTEGER PRIMARY KEY AUTOINCREMENT, blkNumber TEXT, storeyNumber TEXT, unitNumber TEXT, editionNumber TEXT, locationName TEXT, score FLOAT, projectID TEXT, projectName TEXT, creationDate DATETIME default current_timestamp)";
...
- (void)retrieveSavedInformation:(id)sender {
    ...
    if ([filemgr fileExistsAtPath:databasePath] == NO)
    {
        ...
        if (sqlite3_open_v2(charDBPath, &CONQUASDB, SQLITE_OPEN_READWRITE, NULL) == SQLITE_OK) {
            ...
            NSString *selectSQL = @"SELECT ID, blkNumber, storeyNumber, unitNumber, editionNumber, locationName, score, projectID, projectName FROM InternalFinishesSummaryDetails WHERE NOT locationName=\"-Whole Unit-\" ORDER BY editionNumber, blkNumber, storeyNumber, unitNumber, locationName";
            ...
            if (sqlite3_prepare_v2(CONQUASDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
                while (sqlite3_step(statement) == SQLITE_ROW)
                {
                    ...
                    tempProjectID = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 7)];
                    tempProjectName = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 8)];
                    float floatScore = round( [tempScore floatValue] * 100.0 ) / 100.0;
                    
                    UILabel *lblTemp = [[UILabel alloc] initWithFrame:CGRectMake(0, 8 + (37 * countRow) + (8 * countRow), 164, 21)];
                    lblTemp.text = [NSString stringWithFormat: @"%@ Blk %@ #%@-%@ %@ (%1.2f)", tempProjectID, tempBlkNumber, tempStoreyNumber, tempUnitNumber, tempLocationName, floatScore];
                    ...
                    btnTempEdit.frame = CGRectMake(172, (37 * countRow) + (8 * countRow), 50, 37);
                    ...
                    btnTempDelete.frame = CGRectMake(230, (37 * countRow) + (8 * countRow), 50, 37);
            ...
            selectSQL = @"SELECT ID, blkNumber, storeyNumber, unitNumber, editionNumber, locationName, score, projectID, projectName FROM InternalFinishesSummaryDetails WHERE locationName=\"-Whole Unit-\" ORDER BY editionNumber, blkNumber, storeyNumber, unitNumber, locationName";
            ...
            if (sqlite3_prepare_v2(CONQUASDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
                while (sqlite3_step(statement) == SQLITE_ROW)
                {
                    ...
                    UILabel *lblTemp = [[UILabel alloc] initWithFrame:CGRectMake(0, 8 + (37 * countRow) + (8 * countRow), 280, 21)];
                    lblTemp.text = [NSString stringWithFormat: @"Blk %@ Storey %@ Unit %@ (%1.2f) %@: %@", tempBlkNumber, tempStoreyNumber, tempUnitNumber, floatScore, tempProjectID, tempProjectName];
                    lblTemp.adjustsFontSizeToFitWidth = YES;
                    [svSavedInformation addSubview:lblTemp];
...
-(void)sampleEdit:(id)sender  {
    ...
    NSString *selectSQL = [NSString stringWithFormat: @"SELECT blkNumber, storeyNumber, unitNumber, editionNumber, locationName, score, projectID, projectName FROM InternalFinishesSummaryDetails WHERE ID=%i", sampleID];
    const char *query_stmt = [selectSQL UTF8String];
    ...
    if (sqlite3_prepare_v2(CONQUASDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
        if (sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            tempProjectID = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 6)];
            tempProjectName = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 7)];
            ...
            if (intEditionNumber == 6) {
                btnEdition6.selected = TRUE;
                btnEdition7.selected = FALSE;
                btnEdition8.selected = FALSE;
                [btnEdition6 setTitleColor:[UIColor blueColor] forState:UIControlStateSelected];
                [btnEdition7 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                [btnEdition8 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            } else if (intEditionNumber == 7) {
                btnEdition6.selected = FALSE;
                btnEdition7.selected = TRUE;
                btnEdition8.selected = FALSE;
                [btnEdition6 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                [btnEdition7 setTitleColor:[UIColor blueColor] forState:UIControlStateSelected];
                [btnEdition8 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            } else if (intEditionNumber == 8) {
                btnEdition6.selected = FALSE;
                btnEdition7.selected = FALSE;
                btnEdition8.selected = TRUE;
                [btnEdition6 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                [btnEdition7 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                [btnEdition8 setTitleColor:[UIColor blueColor] forState:UIControlStateSelected];
            }
            
            selectSQL = [NSString stringWithFormat: @"SELECT editionNumber, elementType, defectType, numTicks, numChecks FROM InternalFinishesDetails WHERE blkNumber='%@' and storeyNumber='%@' and unitNumber='%@' and locationName='%@' and projectID='%@' and projectName='%@'", tempBlkNumber, tempStoreyNumber, tempUnitNumber, tempLocationName,tempProjectID, tempProjectName];
            query_stmt = [selectSQL UTF8String];
            if (sqlite3_prepare_v2(CONQUASDB, query_stmt, -1, &statement2, NULL) == SQLITE_OK) {
                while (sqlite3_step(statement2) == SQLITE_ROW) {
                    ...
                    int intEditionNumber = [tempEditionNumber floatValue];
                    if (intEditionNumber == 6) {
                        btnEdition6.selected = TRUE;
                        btnEdition7.selected = FALSE;
                        btnEdition8.selected = FALSE;
                    ...
                    if ([tempElementType isEqualToString:@"Ceiling"]) {
                        ...
                        if ([tempDefectType isEqualToString:@"AD"]) {
                            ...
                            txtComponentJPTicks.text = tempNumTicks;
                            txtComponentJPChecks.text = tempNumChecks;
                        ...
                        if ([tempDefectType isEqualToString:@"AD"]) {
                            ...
                            txtDoorJPTicks.text = tempNumTicks;
                            txtDoorJPChecks.text = tempNumChecks;
                        } else if ([tempDefectType isEqualToString:@"MD"]) {
                        ...
                        if ([tempDefectType isEqualToString:@"AD"]) {
                            ...
                            txtMEJPTicks.text = tempNumTicks;
                            txtMEJPChecks.text = tempNumChecks;
                        } else if ([tempDefectType isEqualToString:@"MD"]) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Message" message:[NSString stringWithFormat:@"Data Retrieved Successfully For Block %@ Storey %@ Unit %@ Location %@", tempBlkNumber, tempStoreyNumber, tempUnitNumber, tempLocationName] delegate:self cancelButtonTitle:@"Close" otherButtonTitles: nil];
                [alert show];
            }
...
-(void)sampleDelete:(id)sender {
    ...
    NSString *selectSQL = [NSString stringWithFormat: @"SELECT blkNumber, storeyNumber, unitNumber, editionNumber, locationName, projectID, projectName FROM InternalFinishesSummaryDetails WHERE ID=%i", sampleID];
    const char *query_stmt = [selectSQL UTF8String];
    ...
    if (sqlite3_prepare_v2(CONQUASDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
        if (sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            tempEditionNumber = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 3)];
            tempLocationName = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 4)];
            tempProjectID = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 5)];
            tempProjectName = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 6)];
            
            deleteSQL = [NSString stringWithFormat: @"DELETE FROM InternalFinishesDetails WHERE blkNumber='%@' and storeyNumber='%@' and unitNumber='%@' and editionNumber='%@' and  locationName='%@' and projectID='%@' and projectName='%@';", tempBlkNumber, tempStoreyNumber, tempUnitNumber, tempEditionNumber, tempLocationName, tempProjectID, tempProjectName];
    ...
    deleteSQL = [NSString stringWithFormat: @"DELETE FROM InternalFinishesSummaryDetails WHERE blkNumber=\"%@\" and storeyNumber=\"%@\" and unitNumber=\"%@\" and editionNumber=\"%@\" and locationName=\"-Whole Unit-\" and projectID='%@' and projectName='%@';", tempBlkNumber, tempStoreyNumber, tempUnitNumber, tempEditionNumber, tempProjectID, tempProjectName];
    delete_stmt = [deleteSQL UTF8String];
    sqlite3_prepare_v2(CONQUASDB, delete_stmt, -1, &statement, NULL);
    if (sqlite3_step(statement) == SQLITE_DONE) {}
    sqlite3_finalize(statement);
...
- (IBAction)btnScreenClicked:(id)sender {
    int selectedValue = [sender tag];
    if (selectedValue == 0) {
        svSavedInformation.hidden = NO;
        viewSampleInfomation.hidden = YES;
        viewCeiling.hidden = YES;
        ...
        viewSampleInfomation.hidden = NO;
        viewCeiling.hidden = YES;
        viewComponent.hidden = YES;
        viewDoor.hidden = YES;
        viewFloor.hidden = YES;
...
- (IBAction)calculateScore:(id)sender {
    int intEditionNumber = 7;
    if (btnEdition6.isSelected) {
        intEditionNumber = 6;
    } else if (btnEdition8.isSelected) {
        intEditionNumber = 8;
    }
    ...
    if (intEditionNumber == 6) {
        ...
        componentTotalTicks = [txtComponentADTicks.text floatValue] + [txtComponentAETicks.text floatValue] + [txtComponentFunctionalityTicks.text floatValue] + [txtComponentJPTicks.text floatValue] + [txtComponentMDTicks.text floatValue];
        componentTotalChecks = [txtComponentADChecks.text floatValue] + [txtComponentAEChecks.text floatValue] + [txtComponentFunctionalityChecks.text floatValue] + [txtComponentJPChecks.text floatValue] + [txtComponentMDChecks.text floatValue];
        doorTotalTicks = [txtDoorADTicks.text floatValue] + [txtDoorAETicks.text floatValue] + [txtDoorFunctionalityTicks.text floatValue] + [txtDoorJPTicks.text floatValue] + [txtDoorMDTicks.text floatValue];
        doorTotalChecks = [txtDoorADChecks.text floatValue] + [txtDoorAEChecks.text floatValue] + [txtDoorFunctionalityChecks.text floatValue] + [txtDoorJPChecks.text floatValue] + [txtDoorMDChecks.text floatValue];
        ...
        METotalTicks = [txtMEADTicks.text floatValue] + [txtMEAETicks.text floatValue] + [txtMEFunctionalityTicks.text floatValue] + [txtMEJPTicks.text floatValue] + [txtMEMDTicks.text floatValue];
        METotalChecks = [txtMEADChecks.text floatValue] + [txtMEAEChecks.text floatValue] + [txtMEFunctionalityChecks.text floatValue] + [txtMEJPChecks.text floatValue] + [txtMEMDChecks.text floatValue];
        ...
        if (!([txtComponentJPChecks.text isEqualToString:@""] || [txtComponentJPChecks.text isEqualToString:@"0"] || [txtComponentJPTicks.text isEqualToString:@""])) {
            componentJPWeightage = 2.2;
            componentJPScore = ([txtComponentJPTicks.text floatValue] / [txtComponentJPChecks.text floatValue]) * componentJPWeightage;
        }
        ...
        if (!([txtDoorJPChecks.text isEqualToString:@""] || [txtDoorJPChecks.text isEqualToString:@"0"] || [txtDoorJPTicks.text isEqualToString:@""])) {
            doorJPWeightage = 2;
            doorJPScore = ([txtDoorJPTicks.text floatValue] / [txtDoorJPChecks.text floatValue]) * doorJPWeightage;
        }
        ...
        if (!([txtMEJPChecks.text isEqualToString:@""] || [txtMEJPChecks.text isEqualToString:@"0"] || [txtMEJPTicks.text isEqualToString:@""])) {
            MEJPWeightage = 1;
            MEJPScore = ([txtMEJPTicks.text floatValue] / [txtMEJPChecks.text floatValue]) * MEJPWeightage;
        }
...
- (IBAction)btnEditionClicked:(id)sender {
    int intEditionNumber = [sender tag];
    if (intEditionNumber == 6) {
        btnEdition6.selected = TRUE;
        btnEdition7.selected = FALSE;
        btnEdition8.selected = FALSE;
...
-(IBAction)saveInternalFinishesData:(id)sender {
    ...
    if ([txtBlkNumber.text isEqualToString:@""]) {
        ...
        NSString *strProjectID = txtProjectID.text;
        NSString *strProjectName = txtProjectName.text;
        ...
        if (sqlite3_open(dbpath, &CONQUASDB) == SQLITE_OK)
        {
            insertSQL = [NSString stringWithFormat: @"DELETE FROM InternalFinishesDetails where blkNumber='%@' and storeyNumber='%@' and unitNumber='%@' and editionNumber='%@' and locationName='%@' and projectID='%@' and projectName='%@'", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            if (sqlite3_step(statement) == SQLITE_DONE) {}
            sqlite3_finalize(statement);
            
            insertSQL = [NSString stringWithFormat: @"DELETE FROM InternalFinishesSummaryDetails where blkNumber='%@' and storeyNumber='%@' and unitNumber='%@' and editionNumber='%@' and (locationName='%@' OR locationName='-Whole Unit-') and projectID='%@' and projectName='%@'", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName,strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            if (sqlite3_step(statement) == SQLITE_DONE) {}
            sqlite3_finalize(statement);
            ...
            insertSQL = [NSString stringWithFormat: @"INSERT INTO InternalFinishesDetails(blkNumber, storeyNumber, unitNumber, editionNumber, locationName, elementType, defectType, numTicks, numChecks, projectID, projectName) VALUES (\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", %i, %i, \"%@\", \"%@\");", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, strElementType, strDefectType, intNumTicks, intNumChecks, strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            if (sqlite3_step(statement) == SQLITE_DONE) {}
            sqlite3_finalize(statement);
            ...
            strDefectType = @"JP";
            if ([txtComponentJPTicks.text isEqualToString:@""] || ![self isNumeric:txtComponentJPTicks.text]) {
                intNumTicks = 0;
            } else {
                intNumTicks = [txtComponentJPTicks.text intValue];
            }
            if ([txtComponentJPChecks.text isEqualToString:@""] || ![self isNumeric:txtComponentJPChecks.text]) {
                intNumChecks = 0;
            } else {
                intNumChecks = [txtComponentJPChecks.text intValue];
            }
            insertSQL = [NSString stringWithFormat: @"INSERT INTO InternalFinishesDetails(blkNumber, storeyNumber, unitNumber, editionNumber, locationName, elementType, defectType, numTicks, numChecks, projectID, projectName) VALUES (\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", %i, %i, \"%@\", \"%@\");", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, strElementType, strDefectType, intNumTicks, intNumChecks, strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            ...
            strElementType = @"Door";
            strDefectType = @"JP";
            if ([txtDoorJPTicks.text isEqualToString:@""] || ![self isNumeric:txtDoorJPTicks.text]) {
                intNumTicks = 0;
            } else {
                intNumTicks = [txtDoorJPTicks.text intValue];
            }
            if ([txtDoorJPChecks.text isEqualToString:@""] || ![self isNumeric:txtDoorJPChecks.text]) {
                intNumChecks = 0;
            } else {
                intNumChecks = [txtDoorJPChecks.text intValue];
            }
            insertSQL = [NSString stringWithFormat: @"INSERT INTO InternalFinishesDetails(blkNumber, storeyNumber, unitNumber, editionNumber, locationName, elementType, defectType, numTicks, numChecks, projectID, projectName) VALUES (\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", %i, %i, \"%@\", \"%@\");", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, strElementType, strDefectType, intNumTicks, intNumChecks, strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            ...
            strElementType = @"ME";
            strDefectType = @"JP";
            if ([txtMEJPTicks.text isEqualToString:@""] || ![self isNumeric:txtMEJPTicks.text]) {
                intNumTicks = 0;
            } else {
                intNumTicks = [txtMEJPTicks.text intValue];
            }
            if ([txtMEJPChecks.text isEqualToString:@""] || ![self isNumeric:txtMEJPChecks.text]) {
                intNumChecks = 0;
            } else {
                intNumChecks = [txtMEJPChecks.text intValue];
            }
            insertSQL = [NSString stringWithFormat: @"INSERT INTO InternalFinishesDetails(blkNumber, storeyNumber, unitNumber, editionNumber, locationName, elementType, defectType, numTicks, numChecks, projectID, projectName) VALUES (\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", \"%@\", %i, %i, \"%@\", \"%@\");", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, strElementType, strDefectType, intNumTicks, intNumChecks, strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            ...
            insertSQL = [NSString stringWithFormat: @"INSERT INTO InternalFinishesSummaryDetails(blkNumber, storeyNumber, unitNumber, editionNumber, locationName, score, projectID, projectName) VALUES (\"%@\", \"%@\", \"%@\", \"%@\", \"%@\", %f, \"%@\", \"%@\");", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strLocationName, score,strProjectID, strProjectName];
            insert_stmt = [insertSQL UTF8String];
            sqlite3_prepare_v2(CONQUASDB, insert_stmt, -1, &statement, NULL);
            if (sqlite3_step(statement) == SQLITE_DONE) {}
            sqlite3_finalize(statement);
            ...
            NSString *tempNumTicks;
            NSString *tempNumChecks;
            NSString *selectSQL = [NSString stringWithFormat: @"SELECT elementType, defectType, SUM(numTicks), SUM(numChecks) FROM InternalFinishesDetails where blkNumber=\"%@\" and  storeyNumber=\"%@\" and unitNumber=\"%@\" and editionNumber=\"%@\" and projectID=\"%@\" and projectName=\"%@\" GROUP BY elementType, defectType, projectID, projectName;", strBlkNumber, strStoreyNumber, strUnitNumber, strEditionNumber, strProjectID, strProjectName];
            const char *query_stmt = [selectSQL UTF8String];
...
-(IBAction)resetScore:(id)sender {
    ...
    txtComponentJPTicks.text = @"";
    txtComponentJPChecks.text = @"";
